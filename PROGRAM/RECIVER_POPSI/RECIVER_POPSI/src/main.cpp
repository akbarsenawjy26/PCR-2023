...
const int banyak_slave = 1;
byte address_slave[banyak_slave] = {0x45};
int slave_index = 1;
String from;
...
void loop() {
  ...
  if (i == 0 && !wait_timeout)
  {
    ...
    // reset timeout
    long int milli_sekarang = millis();
    time_skrg = milli_sekarang;
    time_sblm = milli_sekarang;
    wait_timeout = true;
  }

  time_skrg = millis();
  if (time_skrg - time_sblm >= timeout)
  {
    time_sblm = time_skrg;
    if (j == coba_lagi)
    {
